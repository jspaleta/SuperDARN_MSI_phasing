...
int read_attenuators(unsigned int base, int card, int code, int radar){
    ...
    // read PortA and PortB to see if EEPROM output is same as progammed
        ...
        temp=temp + in8(base+portA);
        temp=temp & 0x1f80;
        data=reverse_bits(temp);
        data=data ^ 0x3f;
        return data;
}

int read_data_new(unsigned int base, int card, int code,int radar){
        int data;
        int temp;
        struct  timespec nsleep;
        nsleep.tv_sec=0;
...
int main(int argc, char **argv)
{
  ...
  unsigned int portA1,portB1,portC1,cntrl1 ;
    ...
    if(argc <3 ) {
      ...
      fprintf(stderr,"%s: invoke with radar number (1 or 2) and name\n",argv[0]);
      exit(0);
    }
    radar=atoi(argv[1]);
    strncat(radar_name,argv[2],80);
    switch(radar) {
      case 1:
            portC0=PC_GRP_0;
            portC1=PC_GRP_1;
...
//            sprintf(radar_name,"cvw");
  ...
  for(c=0;c<=19;c++) {
    for(b=0;b<=24;b++) {
      ...
      pcode=read_data_new(IOBASE,c,b,radar);
      tdelay=0.0;
      for(i=0; i<13;i++ ) {
         code= (1 << i);
         if ((pcode & code) == code) tdelay+=delays[i];
      }
      acode=read_attenuators(IOBASE,c,b,radar);
      atten=0.0;
      for(i=0; i<5;i++ ) {
         code= (1 << i);
         if ((acode & code) == code) atten+=attens[i];
      }
      printf("  %s   ,    %4d  ,   %4d   ,   %5d  ,  %8.3lf, %5d   , %8.3lf\n",radar_name,c,b,pcode,tdelay,acode,atten);
      usleep(50);
    }
  }
  return 0;

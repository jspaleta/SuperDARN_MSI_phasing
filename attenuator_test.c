...
int main(int argc, char **argv)
{
  ...
  int ii,i=0,c=31,data=0,index=0,wait_delay=50; 
  unsigned int b=0;
  int last_collect,current_collect,collect=0,beamcode=0,take_data=0,attempt=0,max_attempts=20;
  double fstart;
  double fstop;
  double fstep;
  double atten,pdiff,max_percent_diff,percent_diff,expected_atten,mean_atten,max_atten,min_atten;
  int fnum;
  int radar;
...
//JDS    for (b=0;b<PHASECODES;b++) {
    ...
    sleep(10);
    if (verbose>0) fprintf(stdout,"Measuring attenuations\n");
    for (b=0;b<ATTENCODES;b++) {
      beamcode=b;
...
// Start of Command Block
      ...
      //for(i=0;i<fnum;i++) {
      //  if (verbose > 1) printf("Freq %lf:  Pwr %d:%lf \n",freq[i],b,pwr_mag[i][b]);
      //}
    ...
    for (b=0;b<ATTENCODES;b++) {
      percent_diff=0.0;
      expected_atten=-(double)b*0.5;
      mean_atten=0.0;
      max_atten=-1E10;
      min_atten=1E10;

      for (i=0;i<fnum;i++) {
        atten=pwr_mag[i][b]-pwr_mag[i][0];
        mean_atten+=atten;
        pdiff=fabs((atten-expected_atten)/expected_atten)*100.0;
        if(max_atten < atten) max_atten=atten;
        if(min_atten > atten) min_atten=atten;
        if(pdiff > percent_diff) percent_diff=pdiff;        
      }
      mean_atten=mean_atten/(double)fnum;
      if(percent_diff > max_percent_diff) max_percent_diff=percent_diff;        
      if (verbose > 1) printf("Attencode: %d Mean: %lf Max: %lf Min: %lf Diff%: %5.2g\n",b,mean_atten,max_atten,min_atten,(double)percent_diff);
    }

//    for(i=0;i<fnum;i++) {
//      if (verbose > 1) printf("Freq %lf:  Phase 0:%lf Phase 8191: %lf\n",freq[i],phase[i][0],phase[i][PHASECODES-1]);
//    }
    if (verbose > 1) printf("MaxDiff%: %5.2g\n",(double)max_percent_diff);
    c=-1;
  } // end of Card loop
}

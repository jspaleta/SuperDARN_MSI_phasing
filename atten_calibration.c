...
// Open Socket and initial IO
  ...
  if(stupid_flag) {
    ...
    for (b=0;b<ATTENCODES;b++) {
      ...
      printf("Attencode %d: Freq Index: %d Pwr: %lf Pd: %lf Wait Delay: %d\n",b,fnum-1,atten_pwr_mag[fnum-1][b], pd_new,wait_delay);
    } // end of attencode loop
    for(i=0;i<fnum;i++) {
      if (verbose > 1) printf("Freq %lf:  Pwr 0:%lf Pwr %d: %lf\n",freq[i],atten_pwr_mag[i][0],(int)ATTENCODES-1,atten_pwr_mag[i][ATTENCODES-1]);
    ...
    for(i=0;i<fnum;i++) {
      ...
      if (verbose > 1) printf("Freq %lf:  Pwr 0:%lf Pwr Max: %lf\n",freq[i],atten_pwr_mag[i][0],atten_pwr_mag[i][ATTENCODES-1]);
      fwrite(&i,sizeof(int32),1,attenfile);
      count=fwrite(atten_phase[i],sizeof(double),ATTENCODES,attenfile);
      count=fwrite(atten_pwr_mag[i],sizeof(double),ATTENCODES,attenfile);

...
OPTWRITE_SOURCES=optimize_beam_verify.c MSI_functions.c vna_functions.c utils.c
OPTWRITE_OBJECTS=$(OPTWRITE_SOURCES:.c=.o)
OPTWRITE_EXECUTABLE=optimize_beam_verify
...
$(OPTWRITE_EXECUTABLE): $(OPTWRITE_OBJECTS)
ifeq ($(OS),Linux)
	$(CC) -o $@ $(OPTWRITE_OBJECTS) $(common_libs) $(linux_libs) 
endif
ifeq ($(OS),QNX)
	$(CC) -o $@ $(OPTWRITE_OBJECTS) $(common_libs) $(qnx_libs) 
endif
...
clean:
	rm -rf $(OBJECTS) $(EXECUTABLE) $(SUMMARY_OBJECTS) $(SUMMARY_EXECUTABLE) $(VERIFY_PROGRAMMED_OBJECTS) $(VERIFY_PROGRAMMED_EXECUTABLE) $(VERIFY_OBJECTS) $(VERIFY_EXECUTABLE) $(CONVERT_OBJECTS) $(CONVERT_EXECUTABLE) $(BEAM_OBJECTS) $(BEAM_EXECUTABLE) $(RF_EXECUTABLE) $(RF_OBJECTS) ${FINAL_EXECUTABLE} ${FINAL_OBJECTS} ${OPTBEAM_EXECUTABLE} ${OPTBEAM_OBJECTS} ${OPTWRITE_EXECUTABLE} ${OPTWRITE_OBJECTS}

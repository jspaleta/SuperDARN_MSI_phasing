      ...
      for (b=0;b<BEAMCODES;b++) {
         if (final_beamcodes[c][b] >= 0 ) {
              if(NEW_PMAT) {
                ...
                if(b < num_std_angles) {
                  printf("Radar: %2d  Beam: %4d Phasecode: 0x%08x %4d",radar,b,final_beamcodes[c][b],final_beamcodes[c][b]);
                  printf(" Attencode: 0x%08x %4d\n",final_attencodes[c][b],final_attencodes[c][b]);
                }
